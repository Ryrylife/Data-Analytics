...
row0_0, row0_1, title, _= st.beta_columns((1,0.3,6,1))
# with row0_0:
...
link = ('D:/ry_practice/bank_demographic_fix.csv')
@st.cache()
...
link2 = ('D:/ry_practice/cluster_result_district_fixx.csv') 
@st.cache()
def get_data():
    ...
    two = '00' + data.loc['Quan Ba Dinh':'Quan Thanh Xuan', :]['id'].astype(str)
...
file = open('D:/ry_practice/hanoi.json', encoding='utf8')
@st.cache()
def get_json_data():
    # data = json.load(file)
...
button = st.sidebar.radio('Select a page:', options = ['Overview', 'Sector', 'Methodology'])

# NAVIGATION PANE
if button == 'Overview':
    st.markdown("<h3 style = 'font-weight: bold;color:#AD2A1A;'> Geographic summary </h3>", unsafe_allow_html=True)
    st.markdown("<h4 style = 'color: firebrick;'> Location characteristics </h4>", unsafe_allow_html= True)
    ...
    # SPLIT INTO 3 COLS
    # INSERT SELECTBOX
    bt1, bt2,_,bt3 = st.beta_columns((0.5,0.5,0.1,2))
    with bt1:
        province_select = st.selectbox('Select province', options = df2['province_name_vi'].unique())
    with bt2:
        df_province = df2[df2['province_name_vi']==province_select]
        district_select = st.selectbox('Select district', options = df_province['district'].unique())
    # INSERT A DATA TABLE AND A MAP
    table,_, demo_map = st.beta_columns((2,0.1,2))
    with table:
        df_district = df_province[df_province['district']==district_select]
        df_bftp = df4[df4['id'] == df_district['district_code'].iloc[0]].iloc[:, 3:10]
        ...
        st.markdown("<h4 style = 'color: firebrick;'>Comparing districts sharing the same cluster traits</h4>", unsafe_allow_html=True)
        st.write('')
        #INSERT A CHECKBOX
        cp = st.checkbox('Comparing areas', value = False, key = 'Comparing')  
        # IF IT'S CHECKED, DISPLAY A MAP OF ALL LOCATION WITH THE SAME CHARACTERISTICS
        if cp:
            ...
            with demo_map:
                ...
                dfff = df4[df4.index.isin(df4[df4[group.lower()] == charac].index.unique())]
                ...
                fig_2.update_traces(marker_line=dict(width=1.5, color='LightSlateGrey'))
                st.plotly_chart(fig_2)
                
        else:
            # IF IT'S NOT CHECKED, JUST DISPLAY THE MAP OF THE SELECTED LOCATION
            with demo_map:
                px.set_mapbox_access_token(mapbox_access_token)
                ...
                fig_1 = px.choropleth_mapbox(dff, geojson=geojson, locations='id_district', hover_name = dff.index, 
                                                center={'lat':dff['lat'].values[0], 'lon':dff['lon'].values[0]},
                ...
                fig_1.update_traces(marker_line=dict(width=1.5, color='LightSlateGrey'))
                st.plotly_chart(fig_1)

                # CALCULATE FOR THE FACTS
                df_info = df_district.groupby('district')[['area', 'population', 'pop_density']].agg('sum').reset_index()

                # st.markdown("<h4 style='text-align: left; color: darkgreen;'>Quick facts</h4>", unsafe_allow_html=True)
                fact = st.beta_expander(label = 'Quick facts', expanded = False)
                with fact:
    ...
    st.write('')
    st.markdown("<h4 style='text-align: left; color:firebrick;'>Graphs and charts</h4>", unsafe_allow_html=True)
    ...
    # IF CHECKED THE BOX
    if cp:
        part1, _, part2 = st.beta_columns((2,0.1, 2))

        # CREATE A MULTISELECT OF DISTRICT SATISFIED THE CHARACTERISTICS
        with part1: 
            ...
            dst_compare = st.multiselect('', options = df4[df4[group.lower()] == charac].index.unique().values)
        plot1, _, plot2 = st.beta_columns((1, 0.1, 2))
        if group == 'Age':
            ...
            with plot1:
                for dist in dst_compare:
                    ...
                    def get_cp_age():
                        ...
                        df_age_cp['0_14']=df_age_cp['m0_14']+df_age_cp['f0_14']
                        df_age_cp['15_60']=df_age_cp['m15_60']+df_age_cp['f15_55']
                        df_age_cp['60+']=df_age_cp['m60+']+df_age_cp['f55+']
                        return df_age_cp
                    df_age_1 = get_cp_age()

                    df_age = df_age_1.groupby('district')[['0_14', '15_60', '60+']].agg('sum').reset_index()
                    df_agegroup = pd.melt(df_age[['district', '0_14', '15_60', '60+']], id_vars = ['district'], var_name = 'Age group', value_name= 'value')
                    age2 = px.pie(df_agegroup, title="<b>Age group in {}</b>".format(dist), values='value', color ='Age group', hole = 0.3, color_discrete_map={'0_14':'#17BECF', '15_60':'#66AA00', '60+':'#1F77B4'})
            ...
            with plot2:
                ...
                for dist in dst_compare:
                    ...
                    df_age2melt = pd.melt(df_age2, id_vars = ['commune'], var_name = 'Age group', value_name = 'value')
                    age2bar = px.bar(df_age2melt, title = ' ', x = 'commune', y = 'value', color = 'Age group', color_discrete_sequence=['#17BECF', "#66AA00",'#1F77B4' ])
                    age2bar.update_layout(barmode='stack', xaxis_tickangle = -30, xaxis_title = '', yaxis_title = ' ')
        ...
        if group == 'Pop':
            with plot1:
                for dist in dst_compare:
                    ...
                    def get_cp_age():
                        ...
                        return df_age_cp
                    df_age_1 = get_cp_age()
                    df_age = df_age_1.groupby('district')[['total_male', 'total_female']].agg('sum').reset_index()
                    df_sex = pd.melt(df_age[['district', 'total_male', 'total_female']], id_vars = ['district'], var_name = 'Gender', value_name= 'value')
                    age1 = px.pie(df_sex, title="<b>Gender ratio in {}</b>".format(dist), values='value', color='Gender', hole = 0.3, color_discrete_map={'total_male':'#325A9B', 'total_female':'#DA60CA'})
            ...
            with plot2:
                for dist in dst_compare:
                    ...
                    df_age3melt = pd.melt(df_age3, id_vars = ['commune'], var_name = 'Gender', value_name = 'value')
                    age3bar = px.bar(df_age3melt, title = ' ', x = 'commune', y = 'value', color = 'Gender', color_discrete_sequence=['#325A9B', "#DA60CA"] )
                    age3bar.update_layout(barmode='stack', xaxis_tickangle = -30, xaxis_title = '', yaxis_title = ' ')
        ...
        if group == 'School':
            with plot1: 
                for dist in dst_compare:
                    ...
                    def get_cp_school():
                        df_district = df_province[df_province['district']==dist]
                        df_edu = df_district[['district', 'commune', 'edu_primary', 'edu_secondary', 'edu_high_school', 'edu_higher']]
                        return df_edu
                    df_edu = get_cp_school()
                    df_edu1 = df_edu.groupby('district')[['edu_primary', 'edu_secondary', 'edu_high_school', 'edu_higher']].agg('sum').reset_index()
                    df_edu1melt = pd.melt(df_edu1, id_vars = ['district'], var_name = 'Education level', value_name = 'value')
                    edu1 = px.pie(df_edu1melt, title="<b>Educational stage in {}</b>".format(dist), values = 'value', color = 'Education level', hole = 0.3, color_discrete_map={'edu_primary': '#00CC96', 'edu_secondary':'#FECB52', 'edu_high_school':'#FC6955', 'edu_higher':'rgb(29, 105, 150)'})
            ...
            with plot2:
                for dist in dst_compare:
                    ...
                    def get_cp_school():
                        ...
                        return df_edu
                    df_edu = get_cp_school()
                    df_edu2 = df_edu[['commune', 'edu_primary', 'edu_secondary', 'edu_high_school', 'edu_higher']]
                    df_edu2melt = pd.melt(df_edu2, id_vars = ['commune'], var_name = 'Education level', value_name = 'value')
                    edu2bar = px.bar(df_edu2melt, title = ' ', x = 'commune', y = 'value', color = 'Education level', color_discrete_sequence = ['#00CC96','#FECB52','#FC6955','rgb(29, 105, 150)'])
                    edu2bar.update_layout(barmode='stack', xaxis_tickangle = -30, xaxis_title = '', yaxis_title = ' ')
        ...
        if group == 'Marriage':
            with plot1:
                for dist in dst_compare:
                    ...
                    df_hhs = df_district.groupby('district')[['commune','hh1_2', 'hh_2+']].agg('sum').reset_index()
                    df_hhs1 = pd.melt(df_hhs, id_vars = ['district'], var_name = 'Number of ppl', value_name= 'value')            
            ...
            with plot2:
                for dist in dst_compare:
                    ...
                    df_hhmelt = pd.melt(df_hh, id_vars = ['commune'], var_name = 'Household size', value_name = 'value')
                    hh = px.bar(df_hhmelt, title = ' ', x = 'commune', y = 'value', color = 'Household size', color_discrete_sequence=['#EF553B', "#00A08B"])
                    hh.update_layout(barmode='stack', xaxis_tickangle = -30, xaxis_title = '', yaxis_title = ' ')
                    st.plotly_chart(hh, use_container_width=True)
        else:
            st.write('')

    else:
        button, plot1, plot2 = st.beta_columns((0.4,1,2))
        with button:
            buttonplot = st.radio('', options = ['Household', 'Age group', 'Gender', 'Education'])
        ...
        if buttonplot == 'Age group':
            with plot1:
                df_age = df_age_1.groupby('district')[['0_14', '15_60', '60+']].agg('sum').reset_index()
                df_agegroup = pd.melt(df_age[['district', '0_14', '15_60', '60+']], id_vars = ['district'], var_name = 'Age group', value_name= 'value')
                age2 = px.pie(df_agegroup, title="<b>Age group in {}</b>".format(district_select), values='value', color ='Age group', hole = 0.3, color_discrete_map={'0_14':'#17BECF', '15_60':'#66AA00', '60+':'#1F77B4'})
                st.plotly_chart(age2, use_container_width=True)
            with plot2:
                df_age2 = df_age_1[['commune','0_14', '15_60', '60+' ]]
                df_age2melt = pd.melt(df_age2, id_vars = ['commune'], var_name = 'Age group', value_name = 'value')
        ...
        if buttonplot == 'Gender':
            with plot1:
                df_age = df_age_1.groupby('district')[['total_male', 'total_female']].agg('sum').reset_index()
                df_sex = pd.melt(df_age[['district', 'total_male', 'total_female']], id_vars = ['district'], var_name = 'Gender', value_name= 'value')
                age1 = px.pie(df_sex, title="<b>Gender ratio in {}</b>".format(district_select), values='value', color='Gender', hole = 0.3, color_discrete_map={'total_male':'#325A9B', 'total_female':'#DA60CA'})
                st.plotly_chart(age1, use_container_width=True)
            with plot2:
                df_age3 = df_age_1[['commune','total_male', 'total_female']]
                df_age3melt = pd.melt(df_age3, id_vars = ['commune'], var_name = 'Gender', value_name = 'value')
        ...
        if buttonplot == 'Education':
            ...
            with plot1:
                df_edu1 = df_edu.groupby('district')[['edu_primary', 'edu_secondary', 'edu_high_school', 'edu_higher']].agg('sum').reset_index()
                df_edu1melt = pd.melt(df_edu1, id_vars = ['district'], var_name = 'Education level', value_name = 'value')
                edu1 = px.pie(df_edu1melt, title="<b>Educational stage in {}</b>".format(district_select), values = 'value', color = 'Education level', hole = 0.3, color_discrete_map={'edu_primary': '#00CC96', 'edu_secondary':'#FECB52', 'edu_high_school':'#FC6955', 'edu_higher':'rgb(29, 105, 150)'})
                st.plotly_chart(edu1, use_container_width=True)
            with plot2:
                df_edu2 = df_edu[['commune', 'edu_primary', 'edu_secondary', 'edu_high_school', 'edu_higher']]
                df_edu2melt = pd.melt(df_edu2, id_vars = ['commune'], var_name = 'Education level', value_name = 'value')
                ...
                st.plotly_chart(edu2bar, use_container_width=True)
# PAGE 2 SECTOR
path = ('D:/ry_practice/bank_fixx.csv')
...
if button == 'Sector':
    st.markdown("<h3 style = 'font-weight: bold;color: #AD2A1A;'>Banking sector geo-location analysis</h2>", unsafe_allow_html=True)
    st.markdown("<h4 style = 'color:firebrick;'>First glance at business sector distribution.</h4>", unsafe_allow_html= True)
    ...
    st.write('')
    r1, r2, r3, r4, _  = st.beta_columns((0.3, 0.2, 0.4, 0.15, 0.3))
    file= ('D:/ry_practice/hn_qn_hcm.csv')
    @st.cache()
    def get_data():
        r = pd.read_csv(file)
        r['atm_dens'] = r['population'].divide(r['atm']).round(decimals = 2)
        r['branch_dens'] = r['population'].divide(r['branch']).round(decimals = 2)
        r.replace([np.inf, -np.inf],np.nan, inplace=True)
        return r
    df_count= get_data()

    with r1:
        value = st.selectbox('Select data type', options = ['Top biggest quantity', 'Top smallest quantity', 'Top biggest capacity','Top smallest capacity'])
    with r2:
        outlettype = st.selectbox('Type of outlet', options = ['atm', 'branch'])
    with r3:
        prov = st.selectbox('Select province', options = df_count['province'].unique())
    ...
    # st.markdown("<h5 style = 'font-style: italic;color:#CD594A;'>Communes with .</h5>", unsafe_allow_html= True)
        if prov == 'Thành phố Hồ Chí Minh':
            @st.cache()
            def get_data_hcm():
                df = pd.read_csv('D:/ry_practice/hn_qn_hcm.csv')
                df1_hcm = df[df['province']=='Thành phố Hồ Chí Minh'].reset_index()
            ...
            @st.cache()
            def get_json_hcm():
                with open('D:/ry_practice/hcm.json', encoding = 'utf-8') as f:
    ...
    #         st.plotly_chart(fig_2_hcm, use_container_width = True)
        if prov == 'Tỉnh Quảng Ninh':
            ...
            @st.cache()
            def get_json_qn():
                with open('D:/ry_practice/quang_ninh.json', encoding = 'utf-8') as f:
                    data_qn = json.load(f)
    ...
    #         st.plotly_chart(fig_2_qn, use_container_width = True)
        if prov == 'Thành phố Hà Nội':
            ...
            @st.cache()
            def get_json_hanoi():
                with open('D:/ry_practice/ha_noi.json', encoding = 'utf-8') as f:
                    data_hn = json.load(f)
    ...
    with s2:
        if value == 'Top biggest quantity':
            ...
            if prov == 'Thành phố Hồ Chí Minh':
                df1_hcm_big = df1_hcm[df1_hcm['commune_code'].isin(df_count_1['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df1_hcm_big, data_hcm,{'lat': 10.8113261802206, 'lon': 106.69930197020958}), use_container_width = True)

            if prov == 'Thành phố Hà Nội':
                df3_hanoi_big = df3_hn[df3_hn['commune_code'].isin(df_count_1['commune_code'].astype(str))]
                with s1: 
                    st.plotly_chart(get_viz(df3_hanoi_big, data_hn,{'lat': 21.04501881057594, 'lon': 105.84860999453615} ))

            if prov == 'Tỉnh Quảng Ninh':
                df2_qn_big = df2_qn[df2_qn['commune_code'].isin(df_count_1['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df2_qn_big, data_qn,{'lat': 20.98270863106981, 'lon': 107.01471001641096} )) 
            ...
            figc1.update_layout(title_text='<b>Communes in {} with the most number of {}</b>'.format(prov, outlettype))
            st.plotly_chart(figc1)

        if value == 'Top smallest quantity':
            @st.cache()
            def get_min(outlettype):
                df_min = pd.DataFrame({'{}_0'.format(outlettype): df_prov[df_prov[outlettype]==0].groupby('district').size(),'{}_1'.format(outlettype): df_prov[df_prov[outlettype]==1].groupby('district').size(), '{}>1'.format(outlettype): df_prov[df_prov[outlettype]>1].groupby('district').size()}).reset_index().sort_values('{}_0'.format(outlettype), ascending = False)
            ...
            def get_all_min(outlettype):
                df_all_min = df_prov[(df_prov[outlettype]==0)| (df_prov[outlettype]==1)]
                return df_all_min
            df_all_min = get_all_min(outlettype)

            if prov == 'Thành phố Hồ Chí Minh':
                df1_hcm_small = df1_hcm[df1_hcm['commune_code'].isin(df_all_min['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df1_hcm_small, data_hcm,{'lat': 10.8113261802206, 'lon': 106.69930197020958}), use_container_width = True)

            if prov == 'Thành phố Hà Nội':
                df3_hanoi_small = df3_hn[df3_hn['commune_code'].isin(df_all_min['commune_code'].astype(str))]
                with s1: 
                    st.plotly_chart(get_viz(df3_hanoi_small, data_hn,{'lat': 21.04501881057594, 'lon': 105.84860999453615} ))

            if prov == 'Tỉnh Quảng Ninh':
                df2_qn_small = df2_qn[df2_qn['commune_code'].isin(df_all_min['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df2_qn_small, data_qn,{'lat': 21.15501881057594, 'lon': 107.52860999453615} ))

            figc2 = go.Figure(data=[
                go.Bar(name='{}=0'.format(outlettype), x=df_min['district'] , y=df_min['{}_0'.format(outlettype)], marker_color='#FECB52', text = df_min['{}_0'.format(outlettype)].tolist(), textposition = 'inside', texttemplate='%{text:.0f}'),
                go.Bar(name='{}=1'.format(outlettype), x= df_min['district'] , y=df_min['{}_1'.format(outlettype)], marker_color='rgb(27,158,119)'),
                go.Bar(name='{}>1'.format(outlettype), x= df_min['district'] , y=df_min['{}>1'.format(outlettype)], marker_color='rgb(128,177,211)'),
            ])

            # figc2 = px.bar(df_min, x=df_min['district'], y=['{}_0'.format(outlettype), '{}_1'.format(outlettype), '{}>1'.format(outlettype)])            
            # Change the bar mode
            figc2.update_layout(title_text='<b>Number of communes in each district with the smallest number of {}</b>'.format(outlettype))
        ...
        if value == 'Top biggest capacity':
            ...
            figc3.update_layout(title_text='<b>Communes in {} with the highest capacity of {}</b>'.format(prov, outlettype))
            st.plotly_chart(figc3)

            if prov == 'Thành phố Hồ Chí Minh':
                df1_hcm_big_capa = df1_hcm[df1_hcm['commune_code'].isin(df_count_4['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df1_hcm_big_capa, data_hcm,{'lat': 10.8113261802206, 'lon': 106.69930197020958}), use_container_width = True)

            if prov == 'Thành phố Hà Nội':
                df3_hanoi_big_capa = df3_hn[df3_hn['commune_code'].isin(df_count_4['commune_code'].astype(str))]
                with s1: 
                    st.plotly_chart(get_viz(df3_hanoi_big_capa, data_hn,{'lat': 21.04501881057594, 'lon': 105.84860999453615} ))

            if prov == 'Tỉnh Quảng Ninh':
                df2_qn_big_capa = df2_qn[df2_qn['commune_code'].isin(df_count_4['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df2_qn_big_capa, data_qn,{'lat': 21.15501881057594, 'lon': 107.52860999453615} ))
        ...
        if value == 'Top smallest capacity':
            ...
            if prov == 'Thành phố Hồ Chí Minh':
                df1_hcm_small_capa = df1_hcm[df1_hcm['commune_code'].isin(df_count_5['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df1_hcm_small_capa, data_hcm,{'lat': 10.8113261802206, 'lon': 106.69930197020958}), use_container_width = True)

            if prov == 'Thành phố Hà Nội':
                df3_hanoi_small_capa = df3_hn[df3_hn['commune_code'].isin(df_count_5['commune_code'].astype(str))]
                with s1: 
                    st.plotly_chart(get_viz(df3_hanoi_small_capa, data_hn,{'lat': 21.04501881057594, 'lon': 105.84860999453615} ))

            if prov == 'Tỉnh Quảng Ninh':
                df2_qn_small_capa = df2_qn[df2_qn['commune_code'].isin(df_count_5['commune_code'].astype(str))]
                with s1:
                    st.plotly_chart(get_viz(df2_qn_small_capa, data_qn,{'lat': 21.15501881057594, 'lon': 107.52860999453615} ))
            ...
            figc4 = px.bar(df_count_5, x = 'commune', y = '{}_dens'.format(outlettype), hover_data=['district'], color='{}_dens'.format(outlettype),color_continuous_scale=px.colors.sequential.Viridis,
                    labels={'{}_dens'.format(outlettype):'Density of {}'.format(outlettype),'commune':''}, width = 600, height = 450)
            figc4.update_layout(title_text='<b>Communes in {} with the smallest capacity of {}</b>'.format(prov, outlettype))
    ...
    st.write('')
    bt3, bt4,_,_,_,bt5, bt6, bt7 = st.beta_columns((0.7,0.7,0.5,0.3,0.1,2/3,2/3, 2/3))
    with bt3:
        province_select = st.selectbox('Choosing province', options = df2['province_name_vi'].unique())
    with bt4:
        df_province = df2[df2['province_name_vi']==province_select]
        district_select = st.selectbox('Choosing district', options = df_province['district'].unique())
    with bt5:
        type1 = st.selectbox('Type of information:', options = ['Display all', 'Top 10'])
    if type1=='Display all':
        with bt6:
            type2 = st.selectbox('Type of display', options = ['Commune', 'Bank'])
        with bt7:
            cri_select = st.selectbox('Criteria to display', options = ['Outlet', 'Outlet capacity'])
    if type1 == 'Top 10':
        with bt6:
    ...
    map_image, _, analysis_chart = st.beta_columns((1.9,0.1,2))
    with map_image:
        ...
        fig.add_scattermapbox(lat = df1_1['latitude'],
                                    lon = df1_1['longitude'],
                                    text="Bank name:" + df1_1.bank.astype(str) + "<br>Type:" + df1_1.type_id.astype(str) + "<br>Index:"+df1_1.index.astype(str),
                                    hoverinfo='text',                  
                                    below='',                 
                                    marker_size=9, opacity = 0.8
                            , marker_color='rgb(27,158,119)', 
                            )

        fig.update_layout(coloraxis_showscale= False)
        fig.update_layout(mapbox_style='light', width=530, height=400, margin={"r":0,"t":0,"l":0,"b":0})
        ...
        if type1 == 'Display all':
            if type2 == 'Commune':
                if cri_select == 'Outlet capacity':
                    df2_capa_pre = df_district[['commune', 'population', 'outlet']]
                    df2_capa_pre = df2_capa_pre.copy()
                    df2_capa_pre['outlet_capa']=df2_capa_pre['population'].divide(df2_capa_pre['outlet'])
                    df2_capa = df2_capa_pre[['commune', 'outlet_capa']].sort_values('outlet_capa', ascending = True)
                    ...
                    bar.update_layout( yaxis_title='Number of people being served',xaxis_tickangle=-35, width = 630,  height = 450, title_text = '<b>Outlet capacity of each commune</b>')

                    bar.update_traces(texttemplate='%{text:.2s}', textposition='outside')
                    st.plotly_chart(bar)

                else:
                    df2_outlet = pd.melt(df_district[['commune', 'atm', 'branch']], id_vars = ['commune'], var_name ='type', value_name ='count')
                    bar = px.bar(df2_outlet, x="commune", y="count", color='type', color_discrete_sequence=px.colors.qualitative.D3_r, labels= {'commune':'', 'count':'Number of outlet'})                    
            ...
            if type2 == 'Bank':
                if cri_select == 'Outlet':
                    df3_outlet_df = df1_1.groupby('bank')[['atm', 'branch','office']].agg('sum').reset_index().sort_values('atm', ascending = False)
                    df3_outlet= pd.melt(df3_outlet_df, id_vars = ['bank'], var_name ='type', value_name ='count')
                    bar = px.bar(df3_outlet, x="bank", y="count", color='type', color_discrete_sequence=px.colors.qualitative.D3_r,labels= {'bank':'', 'count':'Number of banks'}, hover_name = 'bank')                    
                    ...
                    df1_2 = df1_1[['bank', 'ward', 'ward_pop']].drop_duplicates(keep = 'first').groupby('bank')['ward_pop'].agg('sum').reset_index()        # if cri_select == 'Branch capacity':
                    df1_3 = df1_1[['bank', 'bank_level','outlet']].groupby(['bank', 'bank_level'])['outlet'].agg('sum').reset_index()        #     df3
                    df1_4 = pd.merge(df1_2, df1_3, on = 'bank')
                    df1_4['outlet_capacity']=(df1_4['ward_pop'].divide(df1_4['outlet'])).round(decimals =2)
                    df1_capa = df1_4[['bank', 'bank_level','outlet_capacity']].sort_values('outlet_capacity', ascending = True)

                    bar = px.bar(df1_capa, x="bank", y="outlet_capacity", 
                                category_orders={"bank_level": ['Big', 'Medium', 'Small']},
...
# https://medium0.com/m/global-identity?redirectUrl=https%3A%2F%2Ftowardsdatascience.com%2Fmake-your-machine-learning-models-come-alive-with-streamlit-48e6eb8e3004
        ...
        if type1 == 'Top 10':
            if type2 == 'Bank':
                if cri_select == 'Outlet':
                    df3_outlet_df = df1_1.groupby('bank')[['atm', 'branch','office']].agg('sum').reset_index().sort_values('atm', ascending = False).head(10)
                    df3_outlet= pd.melt(df3_outlet_df, id_vars = ['bank'], var_name ='type', value_name ='count')
                    bar = px.bar(df3_outlet, x="bank", y="count", color='type', color_discrete_sequence=px.colors.qualitative.D3_r,labels= {'bank':'', 'count':'Number of banks'}, hover_name = 'bank')                    
                    ...
                    df1_4 = pd.merge(df1_2, df1_3, on = 'bank')
                    df1_4['outlet_capacity']=(df1_4['ward_pop'].divide(df1_4['outlet'])).round(decimals =2)
                    df1_capa = df1_4[['bank', 'bank_level','outlet_capacity']].sort_values('outlet_capacity').head(10)
    ...
    st.markdown("<h3 style = 'font-weight: bold; color: #AD2A1A;'>Geodesic buffer map</h2>", unsafe_allow_html=True)
    st.markdown("<h4 style = 'color:firebrick;'>Calculating distance from a specific point (outlet). </h4>", unsafe_allow_html= True)
    ...
    line1,line2, line3, line4, line5, line6 = st.beta_columns((0.7,1,1.2, 1.2, 2.5, 1.2))
    with line1:
        type = st.selectbox('Choose type',  options = df1['type_id'].unique(), index = 0)
        df_type = df1[df1['type_id'] == type]
    with line2:
        bank = st.selectbox('Choose bank', options = df_type['bank'].unique(), index = 6)
        df_bank = df_type[df_type['bank']==bank]
    with line3:
        dist = st.selectbox('Choose district', options = df_bank['district'].unique(), index = 11)
        df_dist = df_bank[df_bank['district']==dist]
    with line4:
        commune = st.selectbox('Choose commune', options = df_dist['ward'].unique())
        df_com = df_dist[df_dist['ward']==commune]
    with line5:
        address = st.selectbox('Choose outlet', options = df_com['dia_chi_cu_the'].unique())
        df_address = df_com[df_com['dia_chi_cu_the']==address]
    with line6:
        buffer = st.number_input('Radius (km)', min_value = 0.0, max_value = 20.0, value = 2.5, step = 0.1, format = "%f")

    map1,_, map2 = st.beta_columns((2, 0.1, 2))
    with map1:
        st.write('')
        ...
        def buffer_map():
            # %timeit
            df1['distance'] = 6367 * 2 * np.arcsin(np.sqrt(np.sin((np.radians(df1['latitude']) - math.radians(lat))/2)**2 + math.cos(math.radians(lat)) * np.cos(np.radians(df1['latitude'])) * np.sin((np.radians(df1['longitude']) - math.radians(lon))/2)**2))            
            df1_pick = df1[(df1['distance']!=0)&(df1['distance']<=buffer)&(df1['type_id']==type)]
            return df1_pick
        df1_pick = buffer_map()
        ...
        fig2.add_trace(go.Scattermapbox(lat = df1_pick[df1_pick['distance']!=0]['latitude'],
                                    lon = df1_pick[df1_pick['distance']!=0]['longitude'],
                                    text="Bank name:" + df1_pick[df1_pick['distance']!=0].bank.astype(str) + "<br>Type:" + df1_pick[df1_pick['distance']!=0].type_id.astype(str)+ "<br>Address:" + df1_pick[df1_pick['distance']!=0].ward.astype(str) + ","+ "<br>" + df1_pick[df1_pick['distance']!=0].district.astype(str),
                                    hoverinfo='text',                  
                                    below='', opacity = 0.7,             
                                    marker_size=9
                            , marker_color='rgb(27,158,119)', 
        ...
        fig2.add_trace(go.Scattermapbox(lat = df1_pick_0['latitude'],
                                    ...
                                    text="Bank name:" + df1_pick_0.bank.astype(str) + "<br>Type:" + df1_pick_0.type_id.astype(str)+ "<br>Address:" + df1_pick_0.ward.astype(str) + ","+ "<br>" + df1_pick_0.district.astype(str),
                                    hoverinfo='text',                  
                                    below='', opacity = 0.8,             
                                    marker_size=11
                            , marker_color='red',))
        ...
        fig2.update_layout(mapbox_style='light', width=500, height=400, margin={"r":0,"t":0,"l":0,"b":0})
        ...
        distance = st.beta_expander('See full table')
        with distance:
            df_show = df1_pick[['bank', 'ward', 'district', 'distance', 'dia_chi_cu_the']].sort_values(by= 'distance')
            df_show['distance'] = df_show['distance'].round(decimals = 2)
            #sort by distance and groupby bank
            components.html(df_show.set_index('bank').to_html(), height = 350, scrolling = True)

    with map2:
        st.write('')
        st.markdown("<h4 style = 'margin-left: 20px;color:firebrick;'>👉 Number of points: %s</h4>" %(len(df1_pick['bank'].index)), unsafe_allow_html= True)
        # st.markdown('👉 Number of points: **%s**' %(len(df1_pick['bank'].index)))
        # st.write('Top 10 banks with the most {} around the selected point'.format(type))
        # # https://plotly.com/python/shapes/
        # # https://plotly.com/python/click-events/
        dfx = pd.DataFrame({'count': df1_pick.groupby('bank').size()}).reset_index().sort_values(by = 'count', ascending = False).head(10)
        fig3 = px.bar(dfx, x='bank', y='count', text = 'count',labels={'count':'Total of outlets', 'bank': ''}, height=450, width = 580, color = 'count', color_continuous_scale= 'Sunset')
        fig3.update_layout(title_text='<b>Top 10 banks with most outlets around the center point</b>',xaxis_tickangle = -40, xaxis_tickfont_size = 11)
